...
def setup():
    """Set up any necessary components"""
    global command_matcher, setup_complete
    try:
        # Import the CommandMatcher (assuming it's defined elsewhere)
        from CommandMatcher import CommandMatcher
        command_matcher = CommandMatcher()
        print("JANI assistant initialized successfully!")
        setup_complete = True
    except Exception as e:
        print(f"Error initializing JANI: {str(e)}")
        setup_complete = False
        
setup()

def return_proper_command(voice_input):
    """Match user voice input to proper command format"""
    global setup_complete, command_matcher
    
    if not setup_complete:
        return {"status": "error", "message": "JANI assistant not properly initialized"}
    
    # First, match the voice input to a command format
    matched_result = command_matcher.process_query(voice_input)
    
    if matched_result["status"] != "success":
        return {
            "status": "error", 
            "message": "I didn't understand that command. Could you try again?"
        }
    
    # Execute the matched command using the existing command processing code
    return process_voice_command(matched_result["command"])
...
def process_voice_command(query):
    ...
    try:
        ...
        # Time
            ...
            unique_hash1 = hashlib.md5(str(time.time()).encode()).hexdigest()[:6]
            note_file = os.path.join(notes_dir, f"note_{unique_hash1}.txt")
            ...
            if not note_content:
                ...
                response["message"] = f"Boss, Note created and saved as note_{unique_hash1}.txt"
        ...
        # Screenshot
            ...
            if cap.isOpened():
                ...
                if ret:
                    # Save image
                    img_name = f"selfie_{dt.datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
        ...
        # Weather
            ...
            filename = f"recording__{unique_hash}.wav"
...
async def voice_command(request: VoiceCommandRequest):
    ...
    try:
        ...
        future = executor.submit(return_proper_command, request.command)
